...
namespace matrix
{
    ...
    typedef SquareMatrix<double, 3> SquareMatrix3f;
    ...
    bool inv(const SquareMatrix<Type, M>& A, SquareMatrix<Type, M>& inv)
    {
        ...
        for (size_t n = 0; n < M; n++) {
            ...
            if (fabs(static_cast<double>(U(n, n))) < DBL_MIN) {
                ...
                for (size_t i = n + 1; i < M; i++) {
                    ...
                    if (fabs(static_cast<double>(U(i, n))) > 1e-8f) {
                        //printf("swapped %d\n",i);
                        U.swapRows(i, n);
                        P.swapRows(i, n);
                        L.swapRows(i, n);
    ...
    template<typename Type, size_t M>
    double det(const SquareMatrix<Type, M>& B)
    {
        SquareMatrix<Type, M> A = B;
        int z, j, i;
        double k, m = 1.0;
        for (z = 0; z < M - 1; z++)
        {
    ...
    typedef SquareMatrix<double, 3> Matrix3f;
