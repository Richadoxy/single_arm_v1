...
CCGFrictionController::CCGFrictionController() {
    // 初始化 L 参数，从文档中提取
    ...
    // friction_params = {1.46662181652836, 2.87362664634089, 2.86603792527201, 1.06665523043001, 
    // 1.58421626091361,2.07803699751541, 0.249230728190171, -0.0526562371229886, 
    // 0.238993565463567, -0.0944960539648475, 0.206371438209122, 0.0748646675997644};
    friction_params = {1.46662181652836, 2.87362664634089, 2.86603792527201, 1.06665523043001, 
...
void CCGFrictionController::computeTau(const std::vector<double>& q, const std::vector<double>& q_dot, 
    std::vector<double>& tau, std::vector<double>& grav_gain, std::vector<double>& fric_gain) {
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        double qdi = q_dot[i];  // 直接用 q_dot 向量
        double sign_qdi = (qdi > 0.0 ? 1.0 : (qdi < 0.0 ? -1.0 : 0.0));
        double fc = friction_params[2 * i];      // 库仑
        double fv = friction_params[2 * i + 1];  // 粘性
